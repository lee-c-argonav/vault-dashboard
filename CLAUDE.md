...
`github.com/OWNER/vault-dashboard` is a **public** repository. Anything
...
- `scripts/check-no-secrets.sh` is the automated gate that enforces all of the
  ...
  [Mandatory pre-push check](#mandatory-pre-push-check)). It runs two layers: a
  deterministic scan (structural secret/path patterns baked in, plus every regex in
  the gitignored `.confidential-patterns` file) and, when the `claude` CLI is
  available, an LLM review of the whole committed tree for names or private content a
  regex would miss.
- **Never hard-code a firm, product, person, or account identifier into the gate
  script itself — the script is public, so that would leak the very thing it guards.**
  Real identifiers live only in `.confidential-patterns`, which is gitignored;
  `.confidential-patterns.example` is the committed placeholder template. To teach the
  gate a new confidential term, add its regex there, not to the script.
